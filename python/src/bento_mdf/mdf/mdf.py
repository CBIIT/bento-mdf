"""
Load and write Model Description Format (MDF) files to & from bento-meta Models.
        ...
        for f in self.files:
            if isinstance(f, str):
                if re.match("(?:file|https?)://", f):
                    ...
                    if not response.ok:
                        self.logger.error(
                            f"Fetching url {response.url} returned code {response.status_code}",
                        )
                        raise ArgError(
                            f"Fetching url {response.url} returned code {response.status_code}",
                        )
    ...
    def create_model(self, raiseError=False):
        ...
        for ent in ChainMap(self._model.nodes, self._model.edges).values():
            if isinstance(ent, Node):
                ...
                if len(ends) > 1:
                    self.logger.warning(
                        f"edge '{hdl}' has more than one Ends pair Src:'{src}',"
                        f"Dst:'{dst}'",
                    )
                end = ends[0]
                ...
                self.logger.error(
                    f"Unhandled entity type {type(ent).__name__} for properties",
                )
                success = False
        prop_of = {}
        ...
        for pname in prop_of:
            for ent in prop_of[pname]:
                ...
                if not spec:
                    self.logger.warning(
                        f"property '{pname}' does not have a corresponding "
                        f"propdef for entity '{ent.handle}'",
                    )
                    continue
                elif key in defns_for:
